...
#### Scenario: Dashboard lifecycle fields are available
- **WHEN** dashboard experiment summary rows are queried
- **THEN** each row includes lifecycle state, handoff status, latest review
  marker timestamp, newest reviewable activity timestamp, and pending-review
  state
